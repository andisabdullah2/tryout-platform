...
export async function POST(request: NextRequest) {
  ...
  const validation = schema.safeParse(body);
  if (!validation.success) {
    return NextResponse.json({ error: "paketId diperlukan" }, { status: 422 });
  }
  ...
  if (sudahBeli) {
    return NextResponse.json({ success: true, message: "Sudah memiliki akses", alreadyOwned: true });
  }
  ...
  return NextResponse.json({
    success: true,
    message: `Akses ke "${paket.judul}" berhasil diaktifkan`,
    orderId,
  });
}
