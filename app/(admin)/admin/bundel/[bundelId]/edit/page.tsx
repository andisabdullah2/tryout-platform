...
export default function EditBundelPage() {
  const params = useParams();
  const router = useRouter();
  const bundelId = params["bundelId"] as string;
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/bundel/${bundelId}`).then((r) => r.json()),
      fetch("/api/tryout?limit=50").then((r) => r.json()),
    ]).then(([bundelRes, paketRes]) => {
      if (bundelRes.success) {
        const bundel = bundelRes.data as BundelData;
        setForm({
          judul: bundel.judul,
          deskripsi: bundel.deskripsi,
          kategori: bundel.kategori,
          harga: Number(bundel.harga),
          thumbnailUrl: bundel.thumbnailUrl ?? "",
          status: bundel.status,
        });
        setSelectedPaketIds(bundel.paket.map((bp) => bp.paketId));
      } else {
        setError("Bundel tidak ditemukan");
      }
      if (paketRes.success) setPaketList(paketRes.data.items);
      setIsLoading(false);
    });
  }, [bundelId]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/bundel/${bundelId}`, {
        method: "PUT",
      ...
      if (!json.success) throw new Error(json.error ?? "Gagal mengupdate bundel");
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Bundel Tryout</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <button type="submit" disabled={isSaving || selectedPaketIds.length === 0}
